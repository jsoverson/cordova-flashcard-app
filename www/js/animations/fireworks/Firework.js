...
define(['animations/fireworks/Particle','animations/fireworks/util'], function (Particle, util) {
  ...
  Firework.prototype.explode = function() {
    ...
    this.onExplode();
    this.exploded = true;
    var power = this.power, velocity, numParticles = this.explosionSize;
    while (numParticles--) {
      velocity = util.fromAngle((numParticles / this.explosionSize) * 2 * Math.PI, this.power * (100 - (Math.random() * 80))/100);
      this.particles.push(new Particle([this.pos[0],this.pos[1]]).launch(velocity));
    }
  };

  Firework.prototype.cleanupParticles = function() {
    var length = this.particles.length, numExpired = 0;
    while (length--) this.particles[length].expired && ++numExpired;
    this.expired = this.particles.length === numExpired && this.exploded;
  };
